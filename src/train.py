"""Training script for headline persuasion-route classifier (multi-class, BERT)."""
...
# --------------------------------------------------------------------------
# Evaluation helper (returns metrics dict)
# --------------------------------------------------------------------------
...
def main():
    parser = argparse.ArgumentParser(description="Train headline persuasion-route classifier")
    parser.add_argument("--data_path",  type=str, required=True, help="Path to JSON dataset")
    parser.add_argument("--test_path",  type=str, default=None,  help="Path to separate test JSON (optional)")
    ...
    for epoch in range(config.num_epochs):
        ...
        epoch_path = os.path.join(epochs_dir, f"epoch_{epoch + 1:02d}.pt")
        torch.save({
            "epoch": epoch + 1,
